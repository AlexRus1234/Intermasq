...
| L3 — smoke.sh | ~75-80% API | ✓ 29 suite-файлов, 147 проверок (после predrel-test-remediation-P2: P2.1 добавил `check_length`/`check_lines` length-ассерты на GET-эндпоинтах); плагин-прокси покрыт (`82-plugins.sh`). Иная метрика — доля эндпоинтов, не строки |
