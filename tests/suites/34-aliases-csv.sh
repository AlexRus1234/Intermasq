...
if require_jwt "DNS aliases — CSV import/export" 4; then
    ...
    # P2.1: CSV is plain text — guard with wc -l via check_lines (>=
    # comparison; check itself is exact-equality and wrong here). Suite 30
    # wrote 4 aliases, so header + >=1 data row (>=2 lines) is the robust
    # non-empty guard.
    check_lines "Alias CSV export has header + >=1 data row" 2
