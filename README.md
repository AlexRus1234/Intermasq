...
- **Бэкап и откат** — скачивание ZIP-архива конфигов, быстрый откат по `.bak` и многоуровневая история версий (N последних состояний в `/etc/intermasq/history/`)
...
| `GET` | `/api/history?file=<path>` | Список сохранённых версий файла | Да |
| `GET` | `/api/history/diff?file=<path>&from=<v>&to=<v\|current>` | Diff между версиями | Да |
| `POST` | `/api/history/restore` | Восстановить файл из версии | Да |
