...
// fuzz_test.go — native Go 1.18+ fuzz targets (FuzzXxx) for the two pure
// text parsers that still live in package main (stage 4 of the modularization
// moved the dhcp-host= / address=/cname= parsers to internal/dnsmasq, where
// their fuzz targets now live): parseArpContent and parseLeasesContent.
// They will follow their parsers to their own package in stage 6.

// The fuzz oracle is "does not panic + structural invariants hold on a
// successful parse" (see логи/Hardening_sweep.md §T1). The parsers
// deliberately accept garbage as legitimate input (returning ok=false or an
// empty collection), so we do NOT build a reference output — we hunt for
// panics, infinite loops, nil-derefs and broken round-trips.
//
