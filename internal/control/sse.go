// Intermasq - Web panel for dnsmasq
// Copyright (C) 2026 AlexRus1234
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program. If not, see <https://www.gnu.org/licenses/>.

// Package control hosts the Server-Sent Events broker for live updates of
// the ARP table and dnsmasq service status, plus the dnsmasq reload/test
// helpers those updates report on.
//
// Clients connect via /api/events (auth in the Authorization header — see
// docs/portability-and-validation.md for why ?token= was removed); the
// eventsHandler still lives in package main until stage 11 and reaches this
// package through the exported Client type + Register/Unregister. The
// broadcaster goroutine (StartBroadcaster) polls the underlying files every
// 5s and pushes an event only when the value actually changes, so a typical
// client receives a few messages per minute, not per second.
package control

import (
	"encoding/json"
	"fmt"
	"os/exec"
	"sync"
	"time"

	"intermask/internal/bins"
	"intermask/internal/initd"
	"intermask/internal/netstate"
	"intermask/internal/stats"
)

// Client is one connected SSE subscriber. The Ch channel is the sink the
// broadcaster writes formatted SSE frames into; it is exported so the
// (still main-package) eventsHandler can read frames off it directly.
type Client struct {
	Ch chan string
}

var (
	clients   = make(map[*Client]bool)
	clientsMu sync.Mutex
)

// Register adds a client to the broadcast set.
func Register(client *Client) {
	clientsMu.Lock()
	clients[client] = true
	clientsMu.Unlock()
}

// Unregister removes a client from the broadcast set.
func Unregister(client *Client) {
	clientsMu.Lock()
	delete(clients, client)
	clientsMu.Unlock()
}

// Broadcast sends an event to every connected client. Non-blocking per
// client: a slow consumer with a full channel is silently skipped — better
// to drop a status update than to block the broadcaster.
func Broadcast(event, data string) {
	clientsMu.Lock()
	defer clientsMu.Unlock()
	msg := fmt.Sprintf("event: %s\ndata: %s\n\n", event, data)
	for c := range clients {
		select {
		case c.Ch <- msg:
		default:
		}
	}
}

// StartBroadcaster spawns a goroutine that polls ARP and dnsmasq status
// every 5s and pushes a delta event when either changes. Designed to be
// called once from main; calling more than once will duplicate events.
func StartBroadcaster() {
	go func() {
		lastArp := ""
		lastStatus := false
		for {
			time.Sleep(5 * time.Second)
			arpJSON, status := pollOnce()
			if arpJSON != lastArp {
				Broadcast("arp", arpJSON)
				lastArp = arpJSON
			}
			if status != lastStatus {
				Broadcast("dnsmasq_status", fmt.Sprintf(`{"active":%v}`, status))
				lastStatus = status
			}
		}
	}()
}

// pollOnce performs a single polling iteration: reads the current ARP
// table and dnsmasq status. Extracted from StartBroadcaster so the
// per-iteration logic is unit-testable without sleeping or spawning a
// goroutine. Returns the JSON-marshalled ARP map and the dnsmasq-active flag.
func pollOnce() (arpJSON string, status bool) {
	arp := netstate.GetArpTable()
	arpJSON = ArpToJSON(arp)
	status = CheckDnsmasqStatus()
	return arpJSON, status
}

// ArpToJSON marshals the ARP map (mac → active) to a JSON string. The
// error is intentionally ignored: the input type is always JSON-safe.
func ArpToJSON(arp map[string]bool) string {
	b, _ := json.Marshal(arp)
	return string(b)
}

// CheckDnsmasqStatus asks the active init-system caller whether the dnsmasq
// service is currently running. Returns false on any caller error.
func CheckDnsmasqStatus() bool {
	return initd.Current().IsActive("dnsmasq")
}

// ReloadDnsmasq runs `dnsmasq --test` first and, on success, asks the
// init-system caller to restart the service. A failed test prevents the
// restart so the running dnsmasq keeps working off the previous (valid)
// config. Reload and test-failure counters are bumped for /metrics.
func ReloadDnsmasq() error {
	testCmd := exec.Command(bins.Dnsmasq(), "--test")
	if testOut, testErr := testCmd.CombinedOutput(); testErr != nil {
		stats.Counters.TestFailures.Add(1)
		return fmt.Errorf("%s", testOut)
	}
	if err := initd.Current().Restart("dnsmasq"); err != nil {
		return err
	}
	stats.Counters.Reloads.Add(1)
	return nil
}
