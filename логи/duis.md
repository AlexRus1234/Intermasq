...
| L4 Playwright UI | 20 specs | батч 1+2 + фазы А,Б закрыты (+dns-aliases-add/bulk-import-text/csv-import/reload-ui к фазе А). Хелпер разбит: `lib/api.ts` → barrel + `api-auth.ts` + `api-hosts.ts`. A5 воспроизведён (`test.fail`, root cause pinned). Остаток — фаза В + батч 4 + SSE + A5-фикс. См. `логи/gap2-batch3-phaseB.md` и предыдущие |
...
### Gap 2 — Playwright (UI тесты) — 2 батча + фазы А,Б ЗАКРЫТЫ, остаток = фаза В + батч 4 + SSE/A5
...
reload-ui. Хелпер разбит на `api-auth.ts` + `api-hosts.ts` + barrel `api.ts`.
Логи: `gap2-playwright-bootstrap.md`, `gap2-batch2-ui-coverage.md`,
`gap2-batch3-phaseA.md`, `gap2-batch3-phaseB.md`.
