...
## Блок C — mutation-pass: ОТЛОЖЕН

Не сделан в этой сессии (по решению оператора). План остался в `Gap_2_finish.md`
§7: throwaway-ветка `mutation-test`, 4-5 точечных мутаций (sortBy / addAlias /
reloadDnsmasq / logout / deleteHost), каждая должна ронять ровно ожидаемый spec.
Ветку не мержить, удалить после. Когда будет время на ~5 раундов
«правка → push → CI `run_e2e_tests=true` → анализ» — отдельно.
...
- **(опционально) mutation-pass** — Блок C, отложен. Эмпирическая уверенность в
  качестве тестов; не блокирует release.
- **infra-specs (опционально):** `setup-screen` (2-я инстанция `:18084`),
  полный `sse-live` (writable arp-file) — оба в `Gap_2_finish.md` §6.7/§6.8.
