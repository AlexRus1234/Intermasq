...
| Фаза | Статус | Файл | Содержание | Трудоёмкость | Когда делать |
|------|:------:|------|------------|--------------|--------------|
| **P1** | ✓ | `predrel-test-remediation-p1.md` | критичное: 6 задач — док-рассинхрон, маскирующие A14/A15-теги, ноль-ассерт тесты, argv-fake для A13, де-корреляция hosts-sort seed | ~1 день | **до старта любого рефакторинга** — закрыто `7a84a2e` (2026-08-02) |
| **P2** | ✓ | `predrel-test-remediation-p2.md` | перед глубоким проходом: 11 задач — body-ассерты на GET, self-seed суитов, sandbox-обход, metricsHandler/audit-тесты, fuzz-фикс | ~1.5 дня | до рефакторинга init/backup/metrics/audit/sse — закрыто `8a489f3`+`04ad007`+`c52c3b0` + follow-up `a8998e0`/`9f2a447` (2026-08-03) |
| **P3** | ✓ | `predrel-test-remediation-p3.md` | полиш: 9 задач — A11/Success-fake, инвертированные комментарии, mutation-friendly селекторы, покрытие endoints | ~1 день | до v1.0 release — закрыто (2026-08-04, см. `predrel-test-remediation-p3-exec.md`) |
