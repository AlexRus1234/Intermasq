# API, аутентификация и RBAC

Intermasq — REST API поверх Gin. Базовый путь `/api`; Swagger UI доступен по
...
Два способа, оба принимаются на всех защищённых эндпоинтах:
...
**10 попыток с одного IP за 1 минуту**, сверх — `429 too_many_attempts`.
...
**Source of truth — бэкенд.** UI лишь прячет admin-кнопки для `user` на основе
...
### DNS-записи (A/CNAME/PTR/TXT) — `auth`
...
### Конфиг dnsmasq — `auth` (часть `admin`)
...
### Безопасность: история, backup, reload — часть `admin`
